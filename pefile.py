...
def extend_size_alignment(size, alignment):
    if size % alignment:
        return int(alignment * math.ceil(float(size)/float(alignment)))
    else:
        return size

def extend_to_size(data, size):
    return data + "\x00"*(size - len(data))

def random_string(_min, _max):
    s = ""
    for _ in xrange(random.randint(_min, _max)):
        s += random.choice( string.ascii_lowercase )
    return s

def data_replace(data, offset, new_data):
...
class SectionStructure(Structure):
    ...
    def xor_data(self, code=0x55):
        data = self.get_data()
        new_data = ""
        for b in data:
            if b:
                new_data += chr(ord(b) ^ code)
            else:
                new_data += "\x00"
    ...
    def wipe_header_of_section(self, wipe_with="\x00"):
        self.pe.__data__ = self.pe.__data__[:self.__file_offset__] + wipe_with*self.sizeof() + self.pe.__data__[self.__file_offset__+self.sizeof():]
...
class PE:
    ...
    def recount_SizeOfImage(self):
        """recount OPTIONAL_HEADER.SizeOfImage

        This field must be equal VirtualAddress+Misc_VirtualSize of last section
        """
    ...
    def delete_last_section(self):
        ...
        name = self.sections[-1].Name
        def should_delete_section(el):
            try:
                if el.Name == name:
                    return False
            except Exception:
                pass
            return True
        self.__structures__ = filter(should_delete_section, self.__structures__)

        self.sections[-1].wipe_header_of_section()
        del self.sections[-1]

        self.recount_SizeOfImage()

    def strip_bount_import(self):
        for i in self.OPTIONAL_HEADER.DATA_DIRECTORY:
            if i.name == "IMAGE_DIRECTORY_ENTRY_BOUND_IMPORT":
                i.Size = 0
                i.VirtualAddress = 0

    def strip_resource(self):
        for i in self.OPTIONAL_HEADER.DATA_DIRECTORY:
            if i.name == "IMAGE_DIRECTORY_ENTRY_RESOURCE":
                i.Size = 0
                i.VirtualAddress = 0

    def data_replace(self, offset, new_data):
        self.__data__ = data_replace(data=self.__data__, offset=offset, new_data=new_data)

    def data_copy(self, copy_from, copy_to, copy_len):
        _data = self.__data__[copy_from:copy_from+copy_len]
        self.data_replace(copy_to, _data)

    def data_insert(self, offset, new_data):
        self.__data__ = self.__data__[:offset] + new_data + self.__data__[offset:]

    def add_last_section(self, size=512):
        self.strip_bount_import()

        size = extend_size_alignment(size=size, alignment=self.OPTIONAL_HEADER.SectionAlignment)
        ...
        section.Name = extend_to_size("."+random_string(3, 7), size=8)
        section.Misc = size
        section.Misc_VirtualSize = size
        section.Misc_PhysicalAddress = size
        section.VirtualAddress = extend_size_alignment(size=self.OPTIONAL_HEADER.SizeOfImage, alignment=self.OPTIONAL_HEADER.SectionAlignment)
        section.SizeOfRawData = size

        section.PointerToRawData = self.sections[-1].PointerToRawData + self.sections[-1].SizeOfRawData
        section.Characteristics = 0xC0000060

        self.__data__ = self.__data__[:section.PointerToRawData] + "\x00"*section.SizeOfRawData + self.__data__[section.PointerToRawData:]
    ...
    def extend_last_section(self, size):
        ...
        self.data_insert(section.PointerToRawData+section.Misc_VirtualSize-size, "\x00"*size)
